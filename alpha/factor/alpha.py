...
class Alphas:
    ...
    def lst_alphas(self):
        return sorted(list(filter(lambda m: m.startswith("factor"), dir(self))))
    ...
    def tst_alphas(self, quantiles=5, alpha_names=None, measure="close"):
        if alpha_names is None:
            alpha_names = self.lst_alphas()
        else:
            alpha_names = alpha_names
        for m in alpha_names:
            ...
            fac = pd.read_hdf(os.path.join(DATA_PATH, "Ashare_data/factor_data/{}_{}.h5".format(self.name, m)), key="v")
            fac.index = pd.to_datetime(fac.index)
            close = pd.read_hdf(os.path.join(DATA_PATH, "Ashare_data/1day_data/pv.h5"), key=measure)

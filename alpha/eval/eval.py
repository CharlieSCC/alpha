...
def alpha_pre_process(alpha):
    ...
    alpha["y_pred"] = alpha["y_pred"].unstack().appply(src_norm, axis=1).stack()
    return alpha
...
def create_full_tear_sheet(alpha):
    ...
    return {"IC": ic.mean(),
            ...
            "ARR": pnl_daily.mean() * 252,
            "AV": pnl_daily.std() * np.sqrt(252),
            "Sharpe": pnl_daily.mean() / pnl_daily.std() * np.sqrt(252),
            "WR": win_rate,
            "MDD": max_drawdown
            }
